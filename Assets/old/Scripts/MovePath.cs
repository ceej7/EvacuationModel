...
[System.Serializable]
public class MovePath : MovePathGeneral
{
    [SerializeField]
    ...
    void Start()
    {
        GetComponent<Animator>().CrossFade(animName, 0.1f, 0);
        fire = false;
    ...
    void Update()
    {
        ...
        Vector3 targetPos = new Vector3(finishPos.x, transform.position.y, finishPos.z);
            ...
            else if (Vector3.Distance(transform.position, finishPos) <= 0.2f)
            {
                ...
                {
                    if (targetPoint == 14 && _WalkPath.TTime >  0f)
                    {
                        if (!_WalkPath.isis)
                        {
                            ...
                            if (temp > 1.25f)
                            {
                                ...
                                targetPoint = 100;
                                finishPos = _WalkPath.getNextPoint(w, 100);
                                finishPos.y = hit.point.y;
                            }
                            else if (temp > 0.5f)
                            {
                                ...
                                targetPoint = 101;
                                finishPos = _WalkPath.getNextPoint(w, 101);
                                finishPos.y = hit.point.y;
                            }
                            else if (temp > 0.25f)
                            {
                                ...
                                targetPoint = 97;
                                finishPos = _WalkPath.getNextPoint(w, 97);
                                finishPos.y = hit.point.y;
                            }
                            else
                            {
                                ...
                                targetPoint = 98;
                                finishPos = _WalkPath.getNextPoint(w, 98);
                                finishPos.y = hit.point.y;
                            }
                        }
                        else
                    ...
                    //dest
                    else if (targetPoint == 16 || targetPoint == 114 || targetPoint == 115)
                    {
                        if (write == false)
                        {
                            _static.Bs1--;
                        ...
                        else
                        {
                            if (targetPoint == 16)
                            {
                                finishPos = _WalkPath.getNextPoint(w, 16);
                                finishPos.y = hit.point.y;
                                targetPoint = 16;
                            }
                            this.gameObject.SetActive(false);
